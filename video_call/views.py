...
class RegisterPeerId(APIView):
    ...
    def post(self, request):
        ...
        if peer_id:
            Peer.objects.update_or_create(user=request.user, defaults={'peer_id': peer_id})
...
class GetOnlineUsers(APIView):
    ...
    def get(self, request):
        peers = Peer.objects.exclude(user=request.user)
...
class GetSignals(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        signals = Signal.objects.filter(receiver=request.user).order_by('-created_at')
...
class GetNotifications(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        notifications = Notification.objects.filter(receiver=request.user, status='pending').order_by('-created_at')
